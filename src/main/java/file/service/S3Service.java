...
public class S3Service {
	...
	public void uploadS3File(MultipartFile file) throws Exception {
		...
		String attachmentFilName = uuid.toString() + "_" + file.getOriginalFilename();
		Long attachmentFileSize = file.getSize();
		
		// Entitiy 변환
		AttachmentFile attachmentfile = AttachmentFile.builder()
												.attachmentFileName(attachmentFilName)
		...
		if(fileNo != null) {
			// 임시 파일 저장
			File uploadFile = new File(attachmentfile.getFilePath() + "//" + attachmentFilName);
